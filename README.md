...
## v0.13 運用司令室
`/arsenal` は、集まったひとことを改善へ使うための「運用司令室」です。毎回 `/entries`、`/review`、`/developer` を行き来しなくても、今日見るべき数、採用中の弾、次にやること、更新コマンドを1画面で確認できます。

- 画面上部に「今日の運用」を置き、新しく入ったひとこと数、未整理の弾候補数、採用中の弾数を表示します。
- 「次にやること」は、見る、1つ採用する、ChatGPTに相談する、Codexに投げる、完了にする、の短い流れに絞っています。
- 採用中の弾を最優先に表示し、ChatGPT用、Codex用、家族に聞く一言をすぐコピーできます。
- 実装後に使う `release.ps1` のコマンドも `/arsenal` からコピーできます。コミットメッセージは壊れにくい英語を推奨しています。
- `/developer` は詳細な開発材料を見る画面として残し、日々の運用判断は `/arsenal` に寄せています。
- ホーム画面は変わりません。家族は引き続き「入れるだけ」です。

## v0.12 弾薬庫の状態を共通保存

v0.12から、`/arsenal` の弾カード状態は Supabase の `ammo_item_statuses` に保存できます。PCで「採用する」にした弾は、iPhone / iPad でも同じ状態で見られるようになります。

- 状態変更は管理者画面 `/arsenal` だけで行います。
- ホーム画面は変わらず、家族は「入れるだけ」です。
- クライアントから Supabase へ直接触らず、Next.js API 経由で保存します。
- Supabase保存に失敗した場合は、一時的に端末内の `localStorage` に残します。

## v0.11 弾薬庫は「候補を選ぶ」画面

`/arsenal` は、自動で改善を決める画面ではありません。直近のひとことから「弾候補」を拾い、最後に使うかどうかは人間が選びます。

- 「今日使える弾」は「弾候補」に弱めています。
- 各カードは「分類案」「優先度案」「次の一手案」として表示します。
- 候補ごとに「採用する」「寝かせる」「無視する」を選べます。
- 採用したものだけが上部の「次に使う弾」に出ます。
- コピー文面は採用済みの弾を中心に使う想定です。

状態は v0.11 では `localStorage` に保存します。状態管理は `AmmoItemStatus` として分けているため、将来はSupabaseに移して複数端末で同期できます。

...
v0.7 では、ホームの「声で入れる」ボタンは録音してからサーバー側で文字起こしする方式です。文字起こし後に入力欄へ入るので、内容を確認してから「保存する」を押してください。録音音声は保存しません。

この機能にはサーバー専用の `OPENAI_API_KEY` が必要です。音声データは文字起こしのためにサーバー経由で OpenAI API へ送られます。医療、お金、住所、子どもの詳細、職場機密などのセンシティブ情報は音声入力しないでください。

iPhone / iPad のキーボードのマイク入力や、iOSショートカットから声で残す場合は `docs/IOS_SHORTCUTS.md` を見てください。
...
v0.7 では「声で入れる」の文字起こしにだけ OpenAI API を使います。週次まとめや分類の分析は引き続きルールベースです。
...
- `OPENAI_API_KEY` は音声文字起こしに使います。クライアントへ出してはいけません
...
- 写真付きで保存できる
- 写真保存に失敗してもテキストは残る
- 「声で入れる」で録音し、文字起こし結果が入力欄に入る
- 文字起こしに失敗してもテキスト入力に戻れる
- 保存後に余計な提案が出ない

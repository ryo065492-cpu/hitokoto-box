...
- 声で残したい場合は、入力欄をタップして iPhone / iPad 標準キーボードのマイクを使う。
- 入力欄には `lang="ja-JP"`、自動大文字化オフ、自動補正オフ、スペルチェックオフを設定し、日本語入力しやすい属性に寄せる。
- Supabase の Table Editor を日常確認に使わせず、アプリ内の `/entries` で保存内容を日本語で確認する。
- `/entries` は閲覧と削除を含むため、`ADMIN_PASSCODE` で保護する。

## v0.7.2 音声入力を無料運用へ戻す判断

- OpenAI API は従量課金のため、家族用の通常入力導線では使わない。
- ホームの「声で入れる」は録音や `/api/transcribe` を呼ばず、入力欄にフォーカスしてキーボードのマイク入力へ誘導する。
- `/api/transcribe` は将来検証用に残すが、通常UIからは呼ばない。
- `OPENAI_API_KEY` は通常利用では不要にする。
